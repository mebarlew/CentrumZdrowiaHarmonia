...
function initHeroAnimations() {
  ...
  if (title) tl.fromTo(title, { opacity: 0, y: 40 }, { opacity: 1, y: 0, duration: 0.8 }, '-=0.3');
  if (subtitle) tl.fromTo(subtitle, { opacity: 0, y: 30 }, { opacity: 1, y: 0, duration: 0.7 }, '-=0.4');

  if (ctas) {
    const buttons = ctas.querySelectorAll('a, button');
    tl.fromTo(buttons, { opacity: 0, y: 20 }, { opacity: 1, y: 0, duration: 0.6, stagger: 0.1 }, '-=0.3');
  }
}
...
function initSectionReveals() {
  ...
  sections.forEach((section) => {
    ...
    const subtitles = section.querySelectorAll('[data-animate="section-subtitle"]');
    ...
    subtitles.forEach((sub) => {
      gsap.fromTo(sub, { opacity: 0, y: 30 }, {
